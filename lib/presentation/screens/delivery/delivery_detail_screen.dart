...
class DeliveryDetailScreen extends StatelessWidget {
  ...
  Future<void> _featureRequest(BuildContext context) async {
    ...
    if (currentCredits < 50) {
      PaywallService.showInsufficientCreditsSheet(
        ...
        message: 'Taşıma ilanını daha fazla kişiye göstermek için kredi satın alıp hemen öne çıkarabilirsin.',
                ...
                children: [
                  imageUrl.isEmpty
                      ? Container(
                          ...
                          child: const Center(child: Icon(Icons.local_shipping, size: 60)),
                        )
                  ...
                  Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text((data['title'] ?? '').toString(), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 10),
                        Text(context.t('Alış: ${data['pickupAddress'] ?? '-'}', 'Pickup: ${data['pickupAddress'] ?? '-'}')),
                        Text(context.t('Bırak: ${data['dropAddress'] ?? '-'}', 'Drop-off: ${data['dropAddress'] ?? '-'}')),
                        if ((data['deliveryTime'] ?? '').toString().isNotEmpty)
                          Text(context.t('Zaman: ${data['deliveryTime']}', 'Time: ${data['deliveryTime']}')),
                        if (description.trim().isNotEmpty) ...[
                          const SizedBox(height: 16),
                          Text(context.t('Açıklama', 'Description'), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                          ...
                          children: isOwner
                              ? [
                                  _btn(context.t('Düzenle', 'Edit'), onTap: () {
                                    Navigator.push(context, MaterialPageRoute(builder: (_) => CreateDeliveryRequestScreen(requestId: requestId)));
                                  }),
                                  _btn(context.t('İlanı Sil', 'Delete Listing'), onTap: () => _deleteRequest(context)),
                              ...
                              : [
                                  _btn(context.t('Mesaj Gönder\n(İlk mesaj 10 kredi)', 'Send Message\n(First message 10 credits)'), onTap: () => _openChat(context, ownerId)),
                                  _btn(context.t('Teklif Ver (5 kredi)', 'Send Offer (5 credits)'), onTap: () async {
                                    final currentCredits = await CreditService().getUserCredits();
                                    if (!context.mounted) return;
                                    if (currentCredits < 5) {
                                      PaywallService.showInsufficientCreditsSheet(
                        ...
                        if (isOwner) ...[
                          ...
                          Text(context.t('Gelen Teklifler', 'Incoming Offers'), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
                          const SizedBox(height: 12),
                          StreamBuilder<QuerySnapshot>(
                            ...
                            builder: (context, offerSnapshot) {
                              ...
                              if (offers.isEmpty) return Text(context.t('Henüz teklif yok.', 'No offers yet.'));
                                      ...
                                      children: [
                                        Expanded(
                                          child: Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: [
                                              Text(price == null ? 'Fiyat belirtilmedi' : 'Teklif: TL $price', style: const TextStyle(fontWeight: FontWeight.w700)),
                                        ...
                                        ElevatedButton(
                                          onPressed: () async {
                                            ...
                                            try {
                                              final chatId = await ChatService().createChatRoom(currentUserId, senderId, requestId);
                                              if (!context.mounted) return;
                                              Navigator.push(
                                                context,
                                                MaterialPageRoute(
  ...
  static Widget _btn(String text, {VoidCallback? onTap}) => ElevatedButton(onPressed: onTap, child: Text(text));
